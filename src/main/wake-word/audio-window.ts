import { WAKE_WORD_HOP_SAMPLES, WAKE_WORD_WINDOW_SAMPLES } from '../../shared/wake-word'
...
export class RollingAudioWindow {
  ...
  append(chunk: Float32Array): void {
    ...
    this.#sampleCount = Math.min(this.#samples.length, this.#sampleCount + chunk.length)
...
export function makeEmbeddingWindows(
  ...
  const windowCount = Math.floor((frameCount - embeddingWindow) / embeddingStride) + 1
  if (windowCount < classifierEmbeddings) return null

  const elementsPerWindow = embeddingWindow * melBins
  const output = new Float32Array(classifierEmbeddings * elementsPerWindow)
