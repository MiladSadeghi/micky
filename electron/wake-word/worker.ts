...
async function initialize(resources: WorkerResources): Promise<void> {
...
async function predict(audio: Float32Array): Promise<void> {
  ...
  if (embeddingOutput.data.length !== CLASSIFIER_EMBEDDINGS * EMBEDDING_DIM) {
    throw new Error(`Unexpected wake-word embedding size: ${embeddingOutput.data.length}`)
  }

  const classifierResult = await classifierSession.run({
    [classifierSession.inputNames[0]]: new ort.Tensor('float32', embeddingOutput.data, [
      1,
      CLASSIFIER_EMBEDDINGS,
      EMBEDDING_DIM
    ])
...
async function handle(message: WorkerRequest): Promise<void> {
  if (message.type === 'initialize') {
    if (Number.isFinite(message.threshold)) {
      threshold = Math.min(0.99, Math.max(0.01, message.threshold))
    }
