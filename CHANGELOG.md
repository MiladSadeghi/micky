...
- پشتیبانی کامل و رسمی از لینوکس (به‌ویژه Ubuntu و Debian) با بسته‌های نصبی `.deb` و فایل پرتابل `.AppImage`. 🐧
- اضافه شدن فرآیند ساخت و انتشار خودکار خروجی‌های لینوکس در GitHub Actions Release.
- امکان بازکردن برنامه‌ها و فایل‌ها در لینوکس با ابزار `open_app` (استفاده از `gtk-launch` و `xdg-open`).
- تشخیص و بررسی خودکار آپدیت‌های نسخه لینوکس (`.deb` و `.AppImage`) در بخش به‌روزرسانی داخل برنامه.

### 🎨 بهترشده

- طراحی و تطبیق اختصاصی پس‌زمینه، کادرها و پنل‌های برنامه برای سیستم مدیریت پنجره در دسکتاپ‌های لینوکس (مانند GNOME و KDE).

### 🛠 رفع‌شده

- ارتقای امنیت مسیرها و مسدودسازی دسترسی ابزارهای عامل به اطلاعات مرورگرها و حلقه‌کلیدها (Keyrings) در لینوکس. 🔐
- اصلاح متن درخواست‌های تأیید اجرای دستور به حالت خنثی و مستقل از سیستم‌عامل.
