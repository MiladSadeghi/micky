...
export default async function afterPack(context) {
  ...
  for (const platformEntry of await readdir(binaryRoot, { withFileTypes: true })) {
    ...
    for (const archEntry of await readdir(platformDirectory, { withFileTypes: true })) {
      if (!archEntry.isDirectory()) continue
      if (arch !== 'universal' && archEntry.name !== arch) {
        await rm(path.join(platformDirectory, archEntry.name), { recursive: true, force: true })
      }
    }
  }
  ...
  if (platform === 'darwin') {
    ...
    for (const entry of await readdir(activeDirectory, { withFileTypes: true })) {
      if (entry.isFile() && /^libonnxruntime\.\d+\.\d+\.\d+\.dylib$/.test(entry.name)) {
